...
class PartyManager(commands.Cog, Leaderboard_Manager, Authenticator, Liege):
    def __init__(self, bot, liege = 317740007605534722, rcrdstoragefile:str = ".\\cogs\\modules\\authenticate.db",
    ...
    async def liege(self, ctx) -> None: #add liege, if none exists
        try:
            ...
            if hasattr(e, 'message'):
                await ctx.reply(f'Error: Something fucked up. \n Diagnosis: {e.message}', mention_author=True)
            else:
    ...
    async def manual_update(self, ctx, user:str): # Manually adds one to records associated with name
        try:
            await ctx.reply(self.update(user), mention_author=True)
        except Exception as e:
            if hasattr(e, 'message'):
                await ctx.reply(f'Error: Something fucked up. \n Diagnosis: {e.message}', mention_author=True)
            else:
    ...
    @commands.command() 
    @commands.check(check_liege)
    async def manual_override(self, ctx, user:str, wish:int) -> None: #records associated with name are changed to wish
        try:
            await ctx.reply(self.override(user, wish), mention_author=True)
        except Exception as e:
            if hasattr(e, 'message'):
                await ctx.reply(f'Error: Something fucked up. \n Diagnosis: {e.message}', mention_author=True)
            else:
    ...
    async def graph(self,ctx) -> None: #give bar graph of current standings
        try:
            ...
            ax.bar(x = list(range(len(x))), height = y, color = 'tab:pink')
